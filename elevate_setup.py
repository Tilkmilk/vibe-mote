...
def looks_like_dev_copy():
    """这个目录是不是**开发副本**（源码仓库）而不是用户装好的那一份。

    为什么需要（真机踩过，很坑）：开发副本被跑起来、并且在**它的**界面上点了「一键优化」，
    于是**计划任务被改成指向开发目录** —— 从此开机启动的是开发副本（配置还不一样），
    用户看到的是"程序怎么又不对了 / 收音又有问题"。仓库里有这些开发脚本、用户那边没有，
    拿它们当指纹最省事。
    """
    for name in ("_package.py", "_build_offline.py", "_verify_zips_match_head.py"):
...
def install_task(run_level="HighestAvailable", name=TASK_NAME):
    # ★ 守卫：开发目录不许改写/安装自启任务（否则会把用户那份的开机启动顶掉）。
    #   确实需要在本机开发目录测试时，设环境变量 VIBEMOTE_ALLOW_DEV_TASK=1 绕过。
    if looks_like_dev_copy() and os.environ.get("VIBEMOTE_ALLOW_DEV_TASK") != "1":
        log("✗ 这个目录看起来是**开发副本**（里面有 _package.py 等开发脚本）——")
        log("  因此不动计划任务，免得把用户那份的开机启动顶掉。")
        log("  真要在开发目录里装任务：设环境变量 VIBEMOTE_ALLOW_DEV_TASK=1 再跑。")
        return False
